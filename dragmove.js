...
(function () {
  ...
  const dragmove = function(target, handle, onStart, onEnd) {
    ...
    if (!_loaded) {
      target.getRootNode().addEventListener(_isTouch ? "touchmove" : "mousemove", function(e) {
        let c = e;
        if (e.touches) {
          c = e.touches[0];
        }
    ...
    handle.addEventListener(_isTouch ? "touchstart" : "mousedown", function(e) {
      e.stopPropagation();
      e.preventDefault();
    ...
    target.getRootNode().addEventListener(_isTouch ? "touchend" : "mouseup", function(e) {   
      if (onEnd && hasStarted) {
