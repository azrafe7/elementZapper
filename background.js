...
chrome.runtime.onMessage.addListener(async (msg, sender, sendResponse) => {
  console.log("[ElementZapper:BG]", msg);
  const { event, data } = msg;

  if (event === "takeScreenshot") {
    console.log("[ElementZapper:BG] taking screenshot...");
    let dataURL = await takeScreenshot();
    console.log("[ElementZapper:BG]", dataURL);
    ...
    console.log("[ElementZapper:BG] opening cropped image in new tab...");
    let dataURL = data;
    let [activeTab] = await chrome.tabs.query({active: true, lastFocusedWindow: true});
